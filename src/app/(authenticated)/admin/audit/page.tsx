...
export default function AuditPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">面試詳細稽核</h1>
        <p className="text-slate-500 mt-1">查閱系統內所有面試紀錄。（僅管理員可存取）</p>
      </div>
      <Card>
        <table className="w-full">
          <thead><tr className="border-b border-slate-200">
            <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">時間</th>
            <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">應徵職位</th>
            <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">面試者 ID</th>
          ...
          <tbody>{ivs.map(iv => (
            <tr key={iv.id} className="border-b border-slate-100 hover:bg-slate-50">
              <td className="py-3 px-4 text-sm text-slate-600">{fmt(iv.started_at)}</td>
              <td className="py-3 px-4 text-sm font-medium text-slate-900">{iv.position}</td>
              <td className="py-3 px-4 text-xs text-slate-400 font-mono">{iv.user_id}</td>
          ...
          ))}{ivs.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-slate-400">尚無面試紀錄</td></tr>}</tbody>
        </table>
      </Card>
      <Modal isOpen={show} onClose={() => setShow(false)} title={det ? `${det.interview.position} — 面試稽核` : ''} size="lg" footer={<><Button variant="ghost" onClick={() => setShow(false)}>取消</Button><Button onClick={() => setShow(false)}>確定執行</Button></>}>
        {det && <div className="space-y-4">
          <div><p className="text-sm font-medium text-slate-500 mb-1">職缺說明</p><p className="text-sm text-slate-700 bg-slate-50 p-3 rounded-lg">{det.interview.job_description}</p></div>
          {det.questions.map(q => <div key={q.question_number} className="bg-slate-50 p-3 rounded-lg">
            <p className="text-sm font-semibold text-teal-700 mb-1">第{q.question_number}題：{q.question_text}</p>
            <p className="text-sm text-slate-600 mb-1">{q.transcript || '(未作答)'}</p>
            <div className="flex gap-4 text-xs text-slate-400"><span>評分：{q.score}</span><span>耗時：{q.duration_seconds}秒</span></div>
          </div>)}
        </div>}
      </Modal>
    </div>

...
export default function ReportListPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">面試表現報告</h1>
        <p className="text-gray-500 mt-1">AI 深度分析您的回答，提供全方位的面試評估。</p>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">尚無面試報告，請先至大廳建立面試！</p>
            <Link href="/lobby">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-600">面試日期</th>
              ...
              <tbody>
                {interviews.map(iv => (
                  <tr key={iv.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4 text-sm text-gray-600">{formatDate(iv.started_at)}</td>
                    <td className="py-3 px-4 text-sm font-medium text-gray-900">{iv.position}</td>
                    <td className="py-3 px-4 text-sm font-bold text-gray-900">{iv.total_score}</td>
